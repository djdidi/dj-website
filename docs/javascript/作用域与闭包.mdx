...
- 在定时器回调函数创建时，闭包中变量 `count` 始终为 `1`，无论 state 如何更新。

> 具体结合 react hooks 原理和 react 声明周期 一起来看。

**解决方式**

- count 加入到 deps 中，重置定时器，会影响定时器准确性
- useRef 存下 countRef
- useRef 存下 latestCallbackRef，useLayoutEffect 重置 latestCallbackRef
- 参考下 ahooks useTimeout
